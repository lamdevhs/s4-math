= Processus de Poisson et File d'Attente
...
On a bien $\H{F_T(+\infty) = 1}$. La densité de probabilité de $\H{T}$
  est donc $\H{F'_T = \gl e^{-\gl t}}$, donc
  $\H{T \sim \mathscr{E}(\gl)}$ et donc $\H{\E(T) = \ifrac{\gl}}$.
...
Tout évènement de probabilité en $\H{\dt^2}$ est supposée négligeable. +
...
\[
  \begin{cases}
  p'_0 + \gl p_0 = \mu p_1  \qquad (S^0) \\
  p'_n + (\gl + \mu) p_{n} = \gl p_{n-1} + \mu p_{n+1}
...
On obtient un système trop complexe dans le cas général. On
  ...
  les $\H{p_i}$ sont des constantes, et donc pour lesquelles
  $\H{p'_i = 0}$ :
\[
  \begin{cases}
  \mu p_1 - \gl p_0 = 0  \qquad (S^0) \\
  \mu p_2 - \gl p_1 = \mu p_1 - \gl p_0 = 0
      \qquad (S^1) \\
  \mu p_{n+1} - \gl p_n = \mu p_n - \gl p_{n-1} = 0
      \qquad (S^n) \ss \all n \gt 0
  \end{cases}
\]
D'où :
...
\]
La condition
  $\H{\sum p_i = p_0 \sum \left ( \frac{\gl}{\mu} \right ) ^k = 1}$
  permet de déduire que nécessairement $\H{\gl < \mu}$,
  et surtout : $\H{\frac{p_0}{1 - \frac{\gl}{\mu}}} = 1$ d'où
  $\H{p_0 = 1 - \frac{\gl}{\mu}}$ et finalement :
\[
  \all n \geq 0 \ss p_n =
...
[horizontal]
{theorem}::
*Loi de la longueur L de la file d'attente en régime stationnaire* : +
$\H{-}$ Si $\H{\gl \geq \mu}$, la file d'attente
  s'allonge indéfiniment. +
$\H{-}$ Sinon,
  $\H{\P(L = n) = \left( \frac{\gl}{\mu} \right) ^n \left( 1 - \frac{\gl}{\mu} \right) \ss \all n \in \sN }$ +
Dans ce cas-là, on remarque que
  $\H{1 + L \sim \mathscr{G}\left( 1 - \frac{\gl}{\mu} \right)}$. +
D'où :
\[
  \E(1 + L) =
  1 + \E(L) =
...
$\H{-}$ $\H{M/M/1/\infty}$ : un unique {lam} puisque la longueur
  ...
  $\H{p_n = \left ( \frac{\gl}{\mu} \right ) ^n p_0}$. +
$\H{-}$ $\H{M/M/\infty/\infty}$ : toujours un unique lambda, mais
...
\[
  ...
  = \frac {p_0}{n!}
    \left( \frac {\gl}{\mu} \right) ^ n
\]
On reconnaît une loi de Poisson $\H{\mathscr{P}(\gl / \mu)}$.
